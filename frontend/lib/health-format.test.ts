...
describe('formatAge', () => {
  ...
  it('picks the shortest honest unit', () => {
    expect(formatAge(NOW / 1000 - 5, NOW)).toBe('5sn önce');
    expect(formatAge(NOW / 1000 - 120, NOW)).toBe('2dk önce');
    expect(formatAge(NOW / 1000 - 7200, NOW)).toBe('2sa önce');
  });
});

describe('formatDetail', () => {
  it('shows latency while healthy', () => {
    expect(formatDetail('ok', NOW / 1000 - 4, 118, null, NOW)).toBe('4sn önce · 118ms');
  });
