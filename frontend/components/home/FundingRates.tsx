...
export default function FundingRates({ data, isLoading, isError }: FundingRatesProps) {
  ...
  return (
    <Panel
      title="Funding Rates"
      /* A countdown and a column header over no rows describe a table that is
         not there. Both are dropped rather than left standing empty. */
      action={
        hasRows ? (
    ...
    >
      {/* A failed feed is not a market with no funding to report — the two must
          not look the same, and only one of them is worth retrying. */}
      {isError ? (
        <div className="flex flex-col items-center justify-center h-40 text-fg-subtle gap-2">
          <AlertCircle className="w-5 h-5" />
          <span className="text-base">Funding rates unavailable</span>
        </div>
      ) : data.length === 0 ? (
        <div className="flex flex-col items-center justify-center h-40 text-fg-subtle gap-2">
          <AlertCircle className="w-5 h-5" />
          <span className="text-base">No funding rates reported</span>
        </div>
      ) : (
        <div>
